...
const StateManager = {
    loadSessionState() {
        try {
            ...
            const wasConnected = StorageManager.wasConnected();

            allowLoginPrompt = allowLogin;
            loginShown = wasLoggedIn;

            // Se estava conectado e tem credenciais, marca para reconexão
            if (wasConnected && savedCredentials) {
                window.isReconnecting = true;
                stateLogger.log("Detected previous active session - will attempt reconnection");
            }

            stateLogger.log("Loaded session state", { wasLoggedIn, allowLogin, wasConnected });
        } catch (e) {
    ...
    saveConnectionState() {
        try {
            ...
            StorageManager.setWasConnected(isConnected);
            stateLogger.log("Saved connection state", { isConnected });
        } catch (e) {
            stateLogger.error("Error saving connection state", e);
        }
    },
...
function updateConnectionState(state) {
    ...
    switch (state) {
        ...
        case "CONNECTED":
            ...
            StateManager.saveConnectionState();
