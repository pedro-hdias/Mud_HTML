...
class MudSession:
    ...
    def add_websocket(self, ws: WebSocket):
        ...
        self.touch()
        logger.debug(f"Session {self.session_id}: WebSocket added (total: {len(self.websocket_clients)})")
    
    def remove_websocket(self, ws: WebSocket):
        """Remove um cliente WebSocket desta sessão"""
        if ws in self.websocket_clients:
            self.websocket_clients.remove(ws)
            logger.debug(f"Session {self.session_id}: WebSocket removed (remaining: {len(self.websocket_clients)})")
    ...
    async def broadcast_state(self, state: ConnectionState):
        ...
        self.state = state
        log_state_change(previous_state, state, f"session_{self.session_id}")
        ...
        for ws in list(self.websocket_clients):
            try:
                logger.debug(f"Session {self.session_id}: Sending state {state.value} to client")
                await ws.send_json(message)
            except Exception as e:
                logger.warning(f"Session {self.session_id}: Failed to send state to client, marking for removal: {e}")
    ...
    async def broadcast_message(self, message: dict):
        ...
        for ws in list(self.websocket_clients):
            try:
                ...
                logger.warning(f"Session {self.session_id}: Failed to send message to client, marking for removal: {e}")
                disconnected_clients.append(ws)
    ...
    def connect_to_mud(self) -> bool:
        ...
        try:
            logger.debug(f"Session {self.session_id}: Opening TCP socket to {MUD_HOST}:{MUD_PORT}")
            self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.socket.connect((MUD_HOST, MUD_PORT))
            self.socket.setblocking(False)
            logger.info(f"Session {self.session_id}: TCP socket connected")
            self.touch()
            return True
        except Exception as e:
            logger.exception(f"Session {self.session_id}: TCP socket connection failed: {e}")
            self.socket = None
            return False
    ...
    def close_mud_socket(self):
        ...
        if self.socket:
            try:
                logger.debug(f"Session {self.session_id}: Closing TCP socket")
                self.socket.close()
                logger.debug(f"Session {self.session_id}: TCP socket closed")
            except Exception as e:
                logger.exception(f"Session {self.session_id}: TCP socket close failed: {e}")
            self.socket = None
    ...
    def send_to_mud(self, data: bytes):
        ...
        if self.socket:
            try:
                logger.debug(f"Session {self.session_id}: Sending {len(data)} bytes to MUD")
                self.socket.sendall(data)
                self.touch()
            except Exception as e:
                logger.exception(f"Session {self.session_id}: Socket send failed: {e}")
                raise
    ...
    def receive_from_mud(self, buffer_size=4096) -> bytes:
        ...
        if self.socket:
            try:
                ...
                if data:
                    logger.debug(f"Session {self.session_id}: Received {len(data)} bytes from MUD")
                    self.touch()
                return data
                ...
                logger.exception(f"Session {self.session_id}: Socket receive failed: {e}")
                raise
        return None
    
    async def disconnect_from_mud(self):
        """Desconecta do MUD e limpa recursos"""
        logger.debug(f"Session {self.session_id}: Disconnecting from MUD")
        
        # Cancela a task de leitura se existir
        if self.reader_task and not self.reader_task.done():
            logger.debug(f"Session {self.session_id}: Cancelling MUD reader task")
            self.reader_task.cancel()
            try:
                await self.reader_task
            except asyncio.CancelledError:
                logger.debug(f"Session {self.session_id}: MUD reader task cancelled")
    ...
    async def mud_reader(self):
        ...
        logger.debug(f"Session {self.session_id}: MUD reader started")
        ...
        while True:
            try:
                ...
                if not data:
                    ...
                    logger.debug(f"Session {self.session_id}: MUD socket closed by server")
                ...
                logger.debug(f"Session {self.session_id}: Received MUD data chunk: {len(text)} chars")
                ...
                logger.exception(f"Session {self.session_id}: Error reading from MUD: {e}")
                await self.disconnect_from_mud()
                await self.broadcast_message({
                    "type": "system",
